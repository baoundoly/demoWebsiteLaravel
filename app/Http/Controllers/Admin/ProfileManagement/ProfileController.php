...
class ProfileController extends Controller
{
    public function profile()
    {
        ...
        return view('admin.profile-management.profile-info.profile',$data);
    }
    ...
    public function edit()
    {
        ...
        return view('admin.profile-management.profile-info.edit',$data);
    }

    public function duplicateEmailCheck(Request $request)
    {
        $exist = User::where('id','!=',auth()->user()->id)->where('email',$request->email)->first();
        if($exist){
    ...
    {
        $exist = User::where('id','!=',auth()->user()->id)->where('mobile_no',$request->mobile_no)->first();
        if($exist){
    ...
    public function update(Request $request)
    {
        ...
        try {
            ...
            return response()->json(['status'=>'success','message'=>'Data Successfully Updated','reload_url'=>route('admin.profile-management.profile-info.profile')]);
        } catch (\Exception $e) {
            DB::rollback();
